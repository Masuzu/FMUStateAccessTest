...
XmlParser::Enu ScalarVariable::getInitial() {
	const char *value = getAttributeValue(XmlParser::att_initial);
	if (!value) {
		XmlParser::Enu causality = getCausality();
		XmlParser::Enu variability = getVariability();
		if(causality == XmlParser::enu_parameter)
			return XmlParser::enu_exact;
		else if(causality == XmlParser::enu_calculatedParameter)
			return XmlParser::enu_calculated;
		else if(causality == XmlParser::enu_input)
			return XmlParser::enu_approx;
		else if(causality == XmlParser::enu_output)
		{
			if(variability == XmlParser::enu_constant)
				return XmlParser::enu_exact;
			else if(variability == XmlParser::enu_discrete || variability == XmlParser::enu_continuous)
				return XmlParser::enu_calculated;
		}
		else if(causality == XmlParser::enu_local)
		{
			if(variability == XmlParser::enu_constant)
				return XmlParser::enu_exact;
			else
