...
std::string getTempResourcesLocation() {
	char *tempPath = getTmpPath();
	std::string resourcesLocation = "file:///";
	resourcesLocation += tempPath;
	resourcesLocation += RESOURCES_DIR;
	free(tempPath);
...
void loadFMU(const char* fmuFileName, const char *tmpPath, FMU &fmu)
{
	// get absolute path to FMU, NULL if not found
	char* fmuPath = getFmuPath(fmuFileName);
	...
	char* xmlPath = (char *) calloc(sizeof(char), strlen(tmpPath) + strlen(XML_FILE) + 1);
...
#endif
	// load the FMU dll
	char* dllPath = (char *) calloc(sizeof(char), strlen(tmpPath) + strlen(DLL_DIR)
...
void outputRow(const FMU *fmu, fmi2Component c, double time, FILE* file, char separator, fmi2Boolean header)
{
